...
<script>
import { reactive, toRefs, toRaw, computed, getCurrentInstance, onMounted } from 'vue'
...
import {
  ...
  export default {
    ...
    setup() {
      ...
      const insternalInstance = getCurrentInstance().appContext.config.globalProperties;
      ...
      function complete(row) {
        ...
        updateTaskStatus(param).then(res => {
          ...
          if (res.code == 0) {
            data.pageNo = 1;
            getTaskList();
            insternalInstance.$Message.success('更新任务状态成功');
          } else {
            insternalInstance.$Message.error(res.msg);
          }
        })
      }
      // 删除任务
      function remove(id) {
        ...
        deleteTask(param).then(res => {
          ...
          if (res.code == 0) {
            data.pageNo = 1;
            getTaskList();
            insternalInstance.$Message.success('任务删除成功');
          } else {
            insternalInstance.$Message.error(res.msg);
          }
        })
      ...
      function handleSubmit(e) {
        ...
        validate().then(async res => {
          ...
          insternalInstance.$Message[resCode === 0 ? 'success' : 'info'](`${ resCode === 0 ? data.title + '成功' : resMsg }`);
        }).catch(err => console.log('error', err));
      }
      // 重要或不重要
      function toggleFav (row, index) {
        if (row.status == 2) {
          insternalInstance.$Message.error('数据已删除');
        } else {
          // is_major: 0:不重要 1:重要
          ...
          updateMark(_data).then(res => {
            ...
            if (res.code == 0) {
              data.pageNo = 1;
              getTaskList();
              insternalInstance.$Message.success('更新标记成功');
            } else {
              insternalInstance.$Message.error(res.msg);
            }
          })
      ...
      onMounted(() => {
        ...
        // console.log('李哥耳洞', insternalInstance)
        getTaskList()
      })
      return {
        ...toRefs(data),
